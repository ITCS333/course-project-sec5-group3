...
async function handleReplyListClick(event) {
  if (event.target.classList.contains('delete-reply-btn')) {
    ...
    if (result.success === true) {
      currentReplies = currentReplies.filter(function(r) { return r.id !== Number(id); });
      renderReplies();
    }
  }
}
...
async function initializePage() {
  ...
  const [topicRes, repliesRes] = await Promise.all([
    fetch('./api/index.php?id=' + currentTopicId).then(function(r) { return r.json(); }),
    fetch('./api/index.php?action=replies&topic_id=' + currentTopicId).then(function(r) { return r.json(); })
  ]);

  currentReplies = (repliesRes.success && Array.isArray(repliesRes.data))
    ? repliesRes.data
    : [];
