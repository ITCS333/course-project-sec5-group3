...
function getResource($db, $id){
    ...
    if(!$resource) sendResponse(["success" => false], 404);
    sendResponse(["success" => true, "data" => $resource]);
}

function createResource($db, $data){
    if(empty($data['title']) || empty($data['link'])) sendResponse(["success" => false], 400);
    if(!filter_var($data['link'], FILTER_VALIDATE_URL)) sendResponse(["success" => false], 400);
...
function updateResource($db, $data){
    ...
    $link = $data['link'] ?? null;
    if(!$title || !$link || !filter_var($link, FILTER_VALIDATE_URL)) sendResponse(["success" => false], 400);
    $stmt = $db->prepare("UPDATE resources SET title = ?, description = ?, link = ? WHERE id = ?");
    $stmt->execute([$title, $data['description'] ?? "", $link, $data['id']]);
...
function createComment($db, $data){
    ...
    if(empty($resourceId) || empty($data['author']) || empty($data['text'])) sendResponse(["success" => false], 400);
    $stmt = $db->prepare("SELECT id FROM resources WHERE id = ?");
    $stmt->execute([$resourceId]);
    if(!$stmt->fetch()) sendResponse(["success" => false], 404);
    $stmt = $db->prepare("INSERT INTO comments_resource(resource_id, author, text) VALUES(?, ?, ?)");
    $stmt->execute([$resourceId, $data['author'], $data['text']]);
...
try {
    if($method === "GET"){
        ...
        if(isset($_GET['comments']) || isset($_GET['resource_id']) || isset($_GET['resourceId'])){
            getComments($db, $_GET['comments'] ?? $_GET['resource_id'] ?? $_GET['resourceId']);
        } elseif($id) {
            getResource($db, $id);
        } else {
            getAllResources($db, $_GET['search'] ?? $_GET['q'] ?? null);
        }
    }
    elseif($method === "POST"){
    ...
    elseif($method === "DELETE"){
        ...
        // التحقق من وجود أي مفتاح يخص التعليقات في الـ GET
        $isCommentRequest = isset($_GET['comment']) || isset($_GET['comment_id']) || 
                            isset($_GET['commentId']) || isset($_GET['comments']);

        if ($isCommentRequest) {
            // نأخذ القيمة من المفتاح المتاح، أو نستخدم id إذا كان المفتاح مجرد علامة (Flag)
            $comment_id = $_GET['comment'] ?: ($_GET['comment_id'] ?: ($_GET['commentId'] ?: ($_GET['comments'] ?: $id)));
            deleteComment($db, $comment_id);
        } elseif ($id) {
            // إذا أرسل id فقط، نتحقق يدوياً هل هو تعليق أولاً؟
            $stmt = $db->prepare("SELECT id FROM comments_resource WHERE id = ?");
            $stmt->execute([$id]);
            if ($stmt->fetch()) {
                deleteComment($db, $id);
